...
static void batch_xonlypub_tweak_randomizer_gen_bitflip(const secp256k1_hash_ctx *hash_ctx, secp256k1_sha256 *sha, unsigned char **args, size_t n_flip, size_t n_bytes) {
...
static void run_batch_xonlypub_tweak_randomizer_gen_tests(void) {
    ...
    for (i = 0; i < n_checks; i++) {
        ...
        for (j = 0; j < COUNT; j++) {
            batch_xonlypub_tweak_randomizer_gen_bitflip(hash_ctx, &sha, args, 0, 32);
            batch_xonlypub_tweak_randomizer_gen_bitflip(hash_ctx, &sha, args, 1, 1);
            batch_xonlypub_tweak_randomizer_gen_bitflip(hash_ctx, &sha, args, 2, 33);
            batch_xonlypub_tweak_randomizer_gen_bitflip(hash_ctx, &sha, args, 3, 32);
        }
